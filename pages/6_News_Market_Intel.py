...
.news-card{background:#1A1A1D;padding:16px 20px;border-radius:8px;margin-bottom:12px;border-left:5px solid;color:#E0E0E0 !important;}
.news-card *{color:#E0E0E0 !important;}
.news-card a{color:#7ECFFF !important;text-decoration:none;font-size:1.05rem;font-weight:600;}
.news-card a:hover{color:#AADDFF !important;text-decoration:underline;}
.news-title{font-size:1.05rem;font-weight:600;line-height:1.5;}
.news-meta{font-size:0.82rem;color:#999 !important;margin-top:6px;}
.news-detail{font-size:0.92rem;margin-top:4px;line-height:1.6;}
.high{border-color:#FF1744;}.medium{border-color:#E6A817;}.low{border-color:#00E676;}
...
with tab1:
    ...
    if df_news.empty:
        ...
        for _, row in df_news.iterrows():
            ...
            # Badge: HIGH=merah terang/putih, MEDIUM=amber gelap/hitam, LOW=hijau/hitam
            if "high" in urg:
                badge_bg, badge_fg = "#FF1744", "#FFFFFF"
            elif "medium" in urg:
                badge_bg, badge_fg = "#B8730A", "#FFFFFF"
            else:
                badge_bg, badge_fg = "#00875A", "#FFFFFF"
            title_link = (
                f'<a href="{row["link"]}" target="_blank" class="news-title">{row["title"]}</a>'
                if row.get("link") else f'<span class="news-title">{row["title"]}</span>'
            )
            st.markdown(
                f'<div class="news-card {css_class}">'
                f'<span style="background:{badge_bg};color:{badge_fg};padding:3px 10px;border-radius:4px;'
                f'font-size:0.78rem;font-weight:700;letter-spacing:0.5px;">'
                f'{row.get("urgency","Low").upper()}</span>&nbsp;&nbsp;{title_link}'
                f'<div class="news-meta">📅 {str(row.get("pub_date",""))[:20]}</div>'
                f'<div class="news-detail">💡 <b>Impact:</b> {row.get("impact","—")}</div>'
                f'<div class="news-detail">✅ <b>Action:</b> {row.get("action","—")}</div>'
                f'</div>',
