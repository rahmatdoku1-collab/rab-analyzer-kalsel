...
.news-card{background:#1A1A1D;padding:14px 16px;border-radius:8px;margin-bottom:10px;border-left:4px solid;color:#E0E0E0 !important;}
.news-card *{color:#E0E0E0 !important;}
.news-card a{color:#7ECFFF !important;text-decoration:none;}
    ...
    if df_news.empty:
        ...
        for _, row in df_news.iterrows():
            ...
            title_link = (
                ...
                if row.get("link") else f'<span>{row["title"]}</span>'
            )
            st.markdown(
