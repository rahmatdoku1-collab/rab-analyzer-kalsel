...
with tab2:
    ...
    if df_items.empty:
        st.warning("Tidak ada item. Cek format file."); st.stop()
...
with tab3:
    ...
    if missing:
        st.error("Belum siap: " + " | ".join(missing))
    else:
        st.success("Semua dokumen siap!")
...
with tab4:
    ...
    k3.markdown(f'<div class="kpi-card" style="border-color:#1E90FF"><b>Peluang Menang</b><br/>'
                f'<span class="kpi-val" style="color:#1E90FF">{peluang}%</span></div>', unsafe_allow_html=True)
...
# ── TAB 5: CV & PROFILE SCANNER ──────────────────────────────────────────────
with tab5:
    st.markdown("### CV & Company Profile Scanner")
    st.caption("Upload PDF CV / Company Profile → AI scoring legalitas, pengalaman, peluang lolos tender")

    cv_file = st.file_uploader("Upload PDF atau TXT", type=["pdf","txt"], key="cv_uploader")
    ...
    if cv_file:
        if st.button("🔍 Scan & Scoring Sekarang", type="primary", use_container_width=True):
            ...
            if len(text.strip()) < 50:
                st.error("Teks terlalu sedikit — PDF mungkin berupa gambar (scanned). Coba file teks."); st.stop()

            with st.spinner("AI menganalisa dokumen..."):
                result = ai_scan_cv(text)

            if "error" in result:
                st.error(f"AI error: {result['error']}"); st.stop()
            ...
            vcolor = "success" if verdict_cv=="KUAT" else ("warning" if verdict_cv=="CUKUP" else "error")
            getattr(st, vcolor)(f"**VERDICT: {verdict_cv}** | Skor Total: {skor_total}/100 | Peluang Lolos: {peluang_cv}%")
            ...
            with cl2:
                if result.get("kekuatan"):
                    ...
                    for k in result["kekuatan"]: st.success(f"✅ {k}")
                if result.get("dokumen_lengkap"):
                    st.markdown("**Dokumen Ada:**")
                    for d in result["dokumen_lengkap"]: st.info(f"📄 {d}")
            with cr2:
                if result.get("kelemahan"):
                    st.markdown("**Kelemahan:**")
                    for k in result["kelemahan"]: st.warning(f"⚠️ {k}")
                if result.get("dokumen_kurang"):
                    st.markdown("**Dokumen Kurang:**")
                    for d in result["dokumen_kurang"]: st.error(f"❌ {d}")

            if result.get("rekomendasi"):
                st.markdown("---")
                st.markdown(f"**Rekomendasi:** {result['rekomendasi']}")
    else:
        st.info("Upload PDF Company Profile atau CV Perusahaan untuk scoring otomatis.")
